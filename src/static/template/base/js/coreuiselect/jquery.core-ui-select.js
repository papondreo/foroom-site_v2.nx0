...
(function ($) {
	...
	$.browser.mobile = (/iphone|ipad|ipod/i.test(navigator.userAgent.toLowerCase()));
    ...
    function CoreUISelect(__elem, __options, __templates) {
        ...
        // templates
		if (this.domSelect.attr("class")) {
			if (this.domSelect.data("width")) {
				containerStruct = '<div class="b-core-ui-select '+this.domSelect.attr("class")+'" style="width:'+this.domSelect.data("width")+'px;"></div>';
			} else {
				containerStruct = '<div class="b-core-ui-select '+this.domSelect.attr("class")+'"></div>';
			}
		}
		else
		{
			if (this.domSelect.data("width")) {
				containerStruct = '<div class="b-core-ui-select" style="width:'+this.domSelect.data("width")+'px;"></div>';
			} else {
				containerStruct = '<div class="b-core-ui-select"></div>';
			}
		}
    ...
    CoreUISelect.prototype.buildUI = function() {
		...
		/* Custom Start */
		var colorsArray = [];
		this.domSelect.find('option').each(function(i) {
			jthis = jQuery(this);
			jthisColor = jthis.data("color");
			colorsArray[i] = jthisColor;
		});
		this.dropdownItem.each(function(i) {
			jthis = jQuery(this);
			if (colorsArray[i] != undefined) {
				jthis.prepend('<span style="background:#'+colorsArray[i]+';"</span>');
				jthis.attr("data-color", colorsArray[i]);
			}
		});
		/* Custom End */
