  ...
  <div class="callback-modal">
    <div class="container">
      <div class="row h-100 justify-content-center">
        <div class="col-8 ui-relative">
          <div v-show="tab === 'map'" class="map_container">
            <!--            <header class="block_header block_header__gray">-->
            <header class="block_header">
              Выберите магазин
              <div class="header-button-group">
                <!--                <span>На карте</span>-->
                <button @click="selectTab('list')">Показать списком</button>
              </div>
            </header>
            <shops-map
              ref="callbackModalShopsMap"
              class="shops-map"
              uniqId="callback-shops-map-modal"
              @selectShop="selectShop"
            />
          </div>

          <div v-show="tab === 'list'" class="shops-list_container">
            <!--            <header class="block_header block_header__gray">-->
            <header class="block_header">
              Выберите магазин
              <div class="header-button-group">
                <button @click="selectTab('map')">Показать на карте</button>
                <!--                <span>Из списка</span>-->
              </div>
            </header>
            <div class="shops-list_body">
              <shops-search
                class="ui-mb-3"
                ref="callbackModalShopsSearch"
                @search="searchShop"
              />
        ...
        <div class="col-4">
          <div class="callback-form_container">
            <header class="block_header">Обратный звонок</header>
            <callback-form class="callback-form_body" @close="closeModal" />
          </div>
        </div>
      </div>
    </div>
...
@Component({
  components: { ShopsSearch, ShopCard, ShopsMap, CallbackForm },
})
