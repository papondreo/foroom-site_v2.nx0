<template>
  <div class="constructor__preview" :class="{constructor__preview_action : actionInfo}">
    ...
    ></div>
    <div
      v-if="system && system.windowImg && !loading"
      class="system-img"
...
export default {
  ...
  data: function () {
    return {
      config,
      publicPath: '/_images/constructor/duo/',
  ...
  computed: {
    ...
    system: function () {
      return this.$getSystem('duo')
    },
    material: function () {
      // console.log('e', this.$store.getters['calcCore/params'].oParams.material.materials[this.template.material])
      //return this.$store.getters['calcCore/material']
      return this.$store.getters['calcCore/params'].oParams.material.materials[this.template.material]
    },
    colorImg: function () {
      if (Object.keys(this.system).length) {
