<template>
  ...
  >
    ...
    <div class="ui-mt-5 ui-mb-4">
      <p class="selected-shop-title">Вам перезвонит {{ shopType }}</p>
      <p class="selected-shop">
        <span class="name">{{ selectedShop.name }}</span>
        по адресу:
        <span class="address">
          {{ selectedShop.city }}, {{ selectedShop.address }}
        </span>

        <shop-schedule
          class="ui-text-left shop-schedule"
