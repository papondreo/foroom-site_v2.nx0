  ...
  <div class="cart-item-card">
    <div class="preview-img">
      <div
        ...
        class="action"
      >
        <p class="action-title">АКЦИЯ «{{ itemParams.current_action.val }}»</p>
        <p class="action-perc">ВАША СКИДКА {{ itemParams.perc.val }}%</p>
      </div>
      <img
        v-for="img in imagesArr"
        :key="img.url"
        :src="img.url"
        alt=""
      />
...
@Component
export default class CartItemCardCornice extends Vue {
  @Prop({ required: true })
  ...
  public publicPath = '/_images/constructor/'
  ...
  oldPrice(key) {
    const mainItems = cartStore.cartItemsGetter
    const items = cartStore.cartItemsRusGetter

    if (items[key]) {
      if (items[key].old_price) {
        if (
          mainItems[key].NumberOfRows &&
          mainItems[key].NumberOfRows == '2'
        ) {
          const price =
            (items[key].old_price.val - -mainItems[key].old_price) *
            items[key].amount.val
          return this.$priceSet(price)
        } else {
          const price = items[key].old_price.val * items[key].amount.val
          return this.$priceSet(price)
        }
      }
      return false
  ...
  price(key) {
    ...
    if (items[key]) {
      if (mainItems[key].NumberOfRows && mainItems[key].NumberOfRows == '2') {
        const price =
          (items[key].price.val - -mainItems[key].price) * items[key].amount.val
        return this.$priceSet(price)
      } else {
        const price = items[key].price.val * items[key].amount.val
        return this.$priceSet(price)
      }
    }
  }
  ...
  get imagesArr() {
    ...
    const data = items[this.i]
    if (data.subtype != 1) {
      const at = data.ArchType ? data.ArchType : 1
      if (data.MountPoint == 2 && at != 2 && at != 3 && data.subtype != 1) {
        arr.push({ url: this.publicPath + 'cornices/bracket.png', val: 1 })
      }

      if (data.NumberOfRows == 2) {
        arr.push({
          url:
            this.publicPath +
            'cornices/' +
            data.currentTemplates[1].type +
            data.currentTemplates[1].subtype +
            '-1_2.png',
          val: 2
        })
      }
...
.action {
  ...
  &-perc {
    ...
    top: -4px;
    position: relative;
    background-color: rgba(#de1000, 0.7);
  }
}
.preview-img {
  flex-shrink: 0;
