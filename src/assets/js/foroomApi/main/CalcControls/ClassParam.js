let ClassParam = function(classItem) {
	...
	this.calcPrice = function (recalc, isReinit = false) {
...
*/
	...
	this.applyParamConditions = function (self, param, key, val) {
		...
		if (key === 'ControlHeight' || key === 'ignore_ControlHeight_2') self.app.controlHeightCorrection(param);
		if (key === 'InsertCount') self.app.InsertCountCorrection(param);
		if (key === 'ControlType' && self.app.ControlTypeCorrection) self.app.ControlTypeCorrection(param);
		// if (key === 'PicDirection' && self.app.InsertPicDirectionCorrection) self.app.InsertPicDirectionCorrection(param);
		...
		if (!self.app.console_mode) {
			...
			if (param.recommended) {
				if (self.conditionRelates && self.conditionRelates[param.alias]) {
					...
					if(param.alias == 'BlockCount' && param.recommended_value < 2)param.recommended_value = 2;
