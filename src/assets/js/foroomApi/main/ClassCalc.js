let ClassCalc = function(foroomApi) {
    ...
    */
            ...
            if (self.json_data.all_data.izd) {
                for (let key in self.json_data.all_data.izd) {
                    ...
                    if (val.max) {
                        ...
                        if (val.max.width
                            && ((
                                this.foroomApi.params.destination != 'fr' &&
                                val.disabled != 1) || (
                                    this.foroomApi.params.destination == 'fr' &&
                                val.disabled == 0))
                        ) {
                        ...
                        if (val.max.height
                            ...
                            // this.max_dimentions[val.ptype].height = 100;
                        }
                    }
                    ...
                    // console.log(val.id, Object.keys(self.json_data.multiple_cost));
                    

                    if (self.json_data.multiple_cost && Object.keys(self.json_data.multiple_cost).includes(String(val.id))) val.multiple_cost = self.json_data.multiple_cost;
    ...
    this.load_template = function(template, cart = false) {
        let self = this;

        this.foroomApi.check_wholesale(cart);


        let type = template.type + template.subtype;
        self.current_item = new self.foroomApi.ClassItems[type]();
        let wholesaleMaterials = [];
        if (this.wholesaleActions && this.wholesaleActions.length) {
            wholesaleMaterials = this.wholesaleActions.map(item => {
                item = {
                    izd: JSON.parse(item.add_izd_types_json),
                    mat: JSON.parse(item.add_mat_ids_json),
                }
                return item;
            }).flat();
        }
        ...
        if (typeof window !== 'undefined' && wholesaleMaterials.length && wholesaleMaterials[0].izd.includes(type) && wholesaleMaterials[0].mat.includes(template.material)) {
            self.current_item.initWholesale(template, function() {}); ///drugoi init
        } else {
            // delete template.id;
            self.current_item.init(template, function() {});
        }
    ...
    this.choose_item_wholesale = function(type, render_container) {
        this.current_item = new this.foroomApi.ClassItems[type]();
        this.current_item.initWholesale();
    };
